...
- `sort_order` (double precision) — manual position for drag-to-reorder; fractional midpoints so a reorder writes only the moved row. New rows use epoch seconds (append to end); pre-migration rows fall back to `created_at`
- `store` (text) — Whole Foods, Costco, Other. Enforced via Postgres CHECK constraint; defaults to Whole Foods
- `created_at`, `updated_at`
...
3. **Grocery List** — a shopping list, shared and realtime, that works fully offline. Items are grouped under three fixed store sections — Whole Foods, Costco, Other (always shown, even empty) — mirroring the category grouping on Inventory; new items default to Whole Foods (or the active store filter, if one is set). Each row has a check-off circle, the item name (inline-renamable via pencil), −/+ quantity steppers (min 1), and a ≡ drag handle to reorder within a section or drag onto a different section's header/rows to move it there (order and store are shared and sync between phones). A "Filter by store" dropdown (All Stores by default) narrows the view to one section at a time. An add bar at the top creates custom items; a trailing number sets the quantity ("Eggs 2" → Eggs ×2). While typing (2+ chars), matching inventory items appear below the bar with their current quantities ("have N") or a "✓ On list" badge — tapping one fills the input, and a submitted name matching an inventory item links `item_id` so the Inventory cart button shows it as on the list. Suggestions read the offline inventory mirror, so they work without signal. A Share button in the header exports the list as text grouped by store (native share sheet on mobile, clipboard on desktop). Checking an item off strikes it through, fades it out, and removes it (soft-delete); an undo bar then shows what was checked off with an Undo button that restores it, persisting until undone, dismissed, or replaced by the next check-off. Items are added from the Inventory screen via a cart button, or typed in directly here. Independent of inventory — checking off does not change inventory quantities.
